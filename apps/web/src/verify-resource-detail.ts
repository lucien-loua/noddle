...
await runVerify("resource detail module (C6)", () => {
  ...
  check("shared constants export tab panel class", workload.includes("DETAIL_TAB_PANEL_CLASS"));
  check(
    "ActiveTabPanel helper exists",
    readFileSync(join(LIB, "active-tab.tsx"), "utf-8").includes("export function ActiveTabPanel"),
  );

  for (const file of DETAIL_ROUTES) {
    const src = readFileSync(join(ROUTES, file), "utf-8");
    check(`${file} uses resourceDetailBeforeLoad`, src.includes("resourceDetailBeforeLoad"));
    check(`${file} uses useLeaveOnDelete`, src.includes("useLeaveOnDelete"));
    check(`${file} uses useDetailTabChange`, src.includes("useDetailTabChange"));
    check(`${file} avoids inline signed-in beforeLoad`, !src.includes("getAuthState"));
  }
  ...
  check(
    ...
    service.includes("lifecyclePollInterval") && database.includes("lifecyclePollInterval"),
  );
  check("service uses ResourceDetailFrame", service.includes("ResourceDetailFrame"));
  check("database uses ResourceDetailFrame", database.includes("ResourceDetailFrame"));
  check(
    "stack uses ResourceDetailFrame",
    readFileSync(join(ROUTES, DETAIL_ROUTES[2]), "utf-8").includes("ResourceDetailFrame"),
  );
  check(
    ...
    service.includes("ActiveTabPanel") &&
      ...
      readFileSync(join(ROUTES, DETAIL_ROUTES[2]), "utf-8").includes("ActiveTabPanel"),
  );
  check(
  ...
  check(
    ...
    !database.includes("function DatabaseHeaderActions") &&
      readFileSync(join(FEATURES, "database/database-header-actions.tsx"), "utf-8").includes(
  ...
  check(
    ...
    (() => {
      const src = readFileSync(join(FEATURES, "database/database-header-actions.tsx"), "utf-8");
      return (
  ...
  check(
    ...
    !(deploySettings.includes("showDeploy") || deploySettings.includes("showReload")) &&
