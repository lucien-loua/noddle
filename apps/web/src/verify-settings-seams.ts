...
await runVerify("settings seams (SSH factory + SettingsList)", () => {
  ...
  const volumes = readFileSync(path.join(SERVER, "backups/volume/volumes.server.ts"), "utf-8");
  check(
  ...
  check(
    ...
    updates.includes("withManagerSession") && !updates.includes("connectToManager()"),
  );
  check("volume mounts use withManagerSession", volumes.includes("withManagerSession"));

  const servers = readFileSync(path.join(FEATURES, "servers/servers-panel.tsx"), "utf-8");
  const accounts = readFileSync(path.join(FEATURES, "accounts/accounts-panel.tsx"), "utf-8");
  const webhooks = readFileSync(path.join(FEATURES, "webhooks/panel.tsx"), "utf-8");
  check(
