...
export const removeAccount = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ removed: true }> => {
    ...
    // The retyped address is re-checked HERE, before any destructive action,
    // and not just in the dialog — that only protects the clients that show
    // it. Same rule as deleting a service and restoring a backup.
    ...
    if (data.confirmEmail !== target.email) {
