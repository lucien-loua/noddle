...
export const triggerDeploy = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ deploymentId: string }> => {
    ...
    // The row is created HERE, not in the worker: the button must return an
    // id right away so the screen can subscribe to the log stream before the
    // build even starts. Same path as the webhook, which queues the same
    // kind of row without going through a session.
