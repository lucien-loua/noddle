...
/** Same width as the topology drawer: a log line does not read in 384px. */
const DRAWER_WIDTH = {
...
/** A section with nothing in it says so, rather than rendering an empty
 *  table head — "no mounts" is an answer, a header row is not. */
      ...
      <TableBody>
        {ports.map((port) => (
          <TableRow key={`${port.containerPort}-${port.published ?? "none"}`}>
            ...
            {/* Exposed and published are not the same fact, and the
                difference IS the answer to "why can I not reach it". */}
...
/**
 * The NAMES of the variables, and the sentence that says why the values
 * are missing.
 *
 * Without the sentence this reads as a rendering bug. With it, it is the
 * product's own rule: values are encrypted at rest, and a screen that
 * printed them would be the hole in that.
 */
...
function Body({ row }: { row: ContainerRow }) {
  ...
  return (
    <>
      ...
      {/* Two tabs and not one scroll: the logs need the full height of the
          panel, and the facts need to be readable without scrolling past
          a thousand lines of output to reach them. */}
      <Tabs
      ...
      >
        ...
        {/* The detail pages' own panel class, and not a hand-rolled one:
            it carries `scroll-fade` and the `data-ending-style:hidden` that
            keeps the OUTGOING panel from staying on screen through its
            transition — a panel set to `flex` outlives its `hidden`. */}
        ...
        <TabsContent className={DETAIL_TAB_PANEL_CLASS} value="logs">
          {/* Mounted only while the tab is open: an SSE follow is a live
              SSH session on the machine, and one opened behind the Details
              tab would keep reading a container nobody is looking at.

              `generation` is the container's own status: when it restarts,
              the tail reattaches instead of staying on the dead stream. */}
...
/**
 * What a row does not have room for: ports, mounts, networks, and the
 * container's own output.
 *
 * A drawer rather than a page: the list is what you came for, and the
 * question asked of one row — "what is this thing, and what is it saying"
 * — is answered beside it, not somewhere you have to navigate back from.
 */
