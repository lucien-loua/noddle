...
function databaseRestoreDescription(target: BackupRestoreTarget | null) {
  if (target?.kind === "run") {
    return (
      <>
        The live data in this database will be <strong>permanently replaced</strong> by the dump
        taken {relativeTime(target.backup.createdAt)}. Noddle writes a safety dump first, so you can
  ...
  return (
    <>
      The live data in this database will be <strong>permanently replaced</strong> by
      {target?.kind === "object" ? (
        <>
          {" "}
...
function volumeRestoreDescription(target: BackupRestoreTarget | null) {
  if (target?.kind === "run") {
    return (
      <>
        The live volume data for this service will be <strong>permanently replaced</strong> by the
        archive taken {relativeTime(target.backup.createdAt)}. Noddle scales the service down,
  ...
  if (target?.kind === "object") {
    return (
      <>
        The live volume data for this service will be <strong>permanently replaced</strong> by{" "}
        <code className="text-xs">{target.objectKey}</code> into volume{" "}
        <code className="text-xs">{target.volumeName}</code>. Noddle scales the service down during
        the restore.
      </>
    );
  }
  ...
  return (
    <>
      The live volume data for this service will be <strong>permanently replaced</strong> by the
      selected archive. Noddle scales the service down during the restore.
    </>
  );
}
