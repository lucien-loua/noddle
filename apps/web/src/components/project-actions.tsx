...
/** The SAME form for creating and renaming: both ask exactly the same
 *  questions, and two copies would diverge at the very first field added. */
function ProjectFormDialog({
  ...
  const save = useMutation({
    ...
    onSuccess: async (created) => {
      ...
      onOpenChange(false);
      // We ENTER the project we just created: it's empty, and staying on
      // the grid would require finding it again to do the only thing that
      // makes sense next — connecting something to it.
      if (created) {
        navigate({
          params: {
  ...
  const form = useAppForm({
    defaultValues: {
      ...
      // biome-ignore lint/suspicious/noUnnecessaryConditions: `existing` is optional, the optional chain can be undefined
      name: existing?.name ?? "",
