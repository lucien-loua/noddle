...
/**
 * The GitLab intake, shared by the per-connection route and the bare one.
 *
 * Hooks Noddle registers carry the connection in their URL. The bare route
 * stays for hooks added by hand, and identifies the connection by trying
...
/** The connection named by the URL, once its own secret matches. */
async function providerById(
  gitProviderId: string,
  token: string
): Promise<string | null> {
  const row = await db.query.gitProviders.findFirst({
    where: eq(gitProviders.id, gitProviderId),
    with: { gitlab: true },
  });
  if (row?.providerType !== "gitlab") {
    return null;
  }
  const secret = secretOf(row);
  return secret && safeEqual(secret, token) ? row.id : null;
}

/** No connection in the URL: every candidate's secret is tried. */
async function providerForToken(token: string): Promise<string | null> {
  ...
  for (const row of rows) {
    const secret = secretOf(row);
    if (secret && safeEqual(secret, token)) {
      return row.id;
    }
  }
  return null;
...
export async function handleGitlabWebhook(
  ...
  const provider = gitProviderId
    ? await providerById(gitProviderId, token)
    : await providerForToken(token);
  if (!provider) {
  ...
  const matching = targets.filter((s) => repositoryMatches(s, repository));
  ...
  if (mr) {
    ...
    const outcomes = await Promise.all(
      matching.map((s) =>
        mr.closed
          ? destroyPreview({ parentServiceId: s.id, prNumber: mr.number })
          : ensurePreview({
