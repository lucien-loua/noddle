 ...
 *
 * Lives on the dashboard process (Bun), not the worker: a shell is a live
 * bidirectional session, not a queue job. Auth mirrors SSE routes —
 * `auth.api.getSession({ headers })` — because TanStack ALS is unavailable
 * on a raw upgrade.
 */
...
export interface TerminalSocketData {
  /** Cookie header from the upgrade request — session lookup in `open`. */
  cookie: string | null;
  forwardedFor: string | null;
  kind: TerminalKind;
  /** Query string already parsed at upgrade time. */
...
/** Client → server control frames (stdin is raw text / binary). */
...
export function parseClientMessage(
  ...
  try {
    ...
    // treat as raw keystrokes that happened to start with `{`
  }
  return { data: raw, type: "raw" };
}
