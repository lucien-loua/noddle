 ...
 */
export type DuplicateSource = NonNullable<Awaited<ReturnType<typeof loadEnvironmentForDuplicate>>>;
...
export async function copyEnvironment(
  ...
  const nameTaken = await db.query.environments.findFirst({
    where: and(eq(environments.projectId, source.projectId), eq(environments.name, data.name)),
  });
  if (nameTaken) {
