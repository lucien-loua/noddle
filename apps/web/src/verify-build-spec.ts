...
import { BUILD_SPEC_FIELDS, buildSpecOf, SERVICE_IDENTITY_FIELDS } from "@noddle/shared/build-spec";
...
await runVerify("service build spec", () => {
  ...
  for (const field of ["publishDirectory", "registryId", "watchPaths", "autoDeploy"]) {
  ...
  const parent = Object.fromEntries(columns.map((c) => [c, `value-of-${c}`])) as Record<
    string,
    unknown
  >;
  const picked = buildSpecOf(parent as Parameters<typeof buildSpecOf>[0]) as Record<
    string,
    unknown
  >;
