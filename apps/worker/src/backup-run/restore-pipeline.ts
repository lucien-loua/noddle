...
export interface RestoreSubject<TRequest, TLoaded> {
  apply: (ctx: DeployContext, loaded: TLoaded, body: NodeJS.ReadableStream) => Promise<void>;
  load: (ctx: DeployContext, request: TRequest) => Promise<TLoaded>;
  missingObjectTarget: string;
  resolveSource: (ctx: DeployContext, request: TRequest, loaded: TLoaded) => Promise<RestoreSource>;
  safetyBackup?: (
    ctx: DeployContext,
    loaded: TLoaded,
...
export async function runRestorePipeline<TRequest, TLoaded>(
  ...
  const { destinationId, objectKey } = await subject.resolveSource(ctx, request, loaded);
