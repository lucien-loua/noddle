...
await runVerify("proxy-config", () => {
  check("hostRule: single domain", hostRule(["app.example.com"]) === "Host(`app.example.com`)");
  check(
  ...
  check("routeLabels: no domains disables Traefik", disabled["traefik.enable"] === "false");
  ...
  check("routeLabels: exposes service on port", plain["traefik.enable"] === "true");
  check(
  ...
  check(
    ...
    multiDomain["traefik.http.middlewares.static-d0-strip.stripprefix.prefixes"] === "/assets",
  );
  check(
  ...
  check(
    ...
    httpsDomain["traefik.http.routers.app-d0.tls.certresolver"] === "letsencrypt",
  );
  ...
  check(
    ...
    stale.includes("traefik.http.routers.svc.rule") &&
      ...
      stale.includes("traefik.http.middlewares.svc-d0-strip.stripprefix.prefixes"),
  );

  check(
    "toLabelArgs: docker --label form",
    toLabelArgs({ "traefik.enable": "true" }).join(",") === "traefik.enable=true",
  );
});
