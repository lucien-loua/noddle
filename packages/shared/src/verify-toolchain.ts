...
import { BUILDKIT_IMAGE, RAILPACK_VERSION, railpackInstallCommand } from "#toolchain";

const SEMVER = /^\d+\.\d+\.\d+$/;
const UNPINNED_INSTALL = /curl[^\n]*railpack\.com\/install\.sh[^\n]*\|\s*\$SUDO sh\s*$/m;
...
const INSTALLER = new URL("../../../installer/install.sh", import.meta.url).pathname;
...
await runVerify("toolchain pinning", () => {
  ...
  check(
    ...
    command.includes(`RAILPACK_VERSION=${RAILPACK_VERSION}`) && command.includes("-E"),
    `got "${command}"`,
  );
