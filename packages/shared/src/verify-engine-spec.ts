...
await runVerify("EngineSpec ownership (C5)", () => {
  ...
  const engines = readFileSync(join(REPO, "database-spec/src/index.ts"), "utf-8");
  const dumpSpec = readFileSync(join(REPO, "backup/src/dump-spec.ts"), "utf-8");
  const restoreSpec = readFileSync(join(REPO, "backup/src/restore-spec.ts"), "utf-8");
  ...
  const runtime = readFileSync(join(REPO, "../apps/worker/src/database/runtime.ts"), "utf-8");

  check(
    "shared has no database-engines facade",
    !existsSync(join(REPO, "shared/src/database-engines.ts")),
  );
  check("DatabaseEngine lives on database-spec", engines.includes("export type DatabaseEngine"));
  check(
  ...
  check("web connection-url has no engine switch", !connectionUrl.includes("switch (engine)"));
  ...
  check(
    ...
    !(password.includes("switch (engine)") || password.includes("switch (database.engine)")),
  );
  check("worker password change uses passwordChangeFor", password.includes("passwordChangeFor"));
  check("password change does not import backup-run", !password.includes("#backup-run"));
  check("password change uses #database-runtime", password.includes("#database-runtime"));
  check(
  ...
  check(
    ...
    dumpSpec.includes("export function dumpSpecFor") && dumpSpec.includes("pg_dump"),
  );
  check(
    "worker dump uses dumpSpecFor",
    backupSubject.includes("dumpSpecFor") && !backupSubject.includes("const DUMP_SPECS"),
  );
  check(
    "restore specs live in @noddle/backup/restore-spec",
    restoreSpec.includes("export function restoreSpecFor") && restoreSpec.includes("pg_restore"),
  );
  check(
