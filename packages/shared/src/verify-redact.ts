...
await runVerify("url credential redaction", () => {
  ...
  check(
    ...
    !redactUrlCredentials(`Cloning ${cloneUrl}\nSubmodule ${cloneUrl}\n`).includes(TOKEN),
  );
  ...
  check(
    ...
    redactUrlCredentials("https://github.com/org/app.git") === "https://github.com/org/app.git",
  );

  check(
    "an email address is not a URL credential",
    redactUrlCredentials("author: jane@example.com") === "author: jane@example.com",
  );
