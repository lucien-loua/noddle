...
await runVerify("gitlab oauth", async () => {
  ...
  // ── expiry, which is the whole difference from GitHub ────────────────
  check("a token with time left is not refreshed", !needsRefresh(NOW + 600_000, NOW));
  ...
  // GitLab rotates the refresh token: keeping the old one means the NEXT
  // refresh fails, and the connection dies silently a couple of hours on.
  check(
  ...
  check(
    ...
    repos.length === 1 && repos[0]?.fullName === "acme/api" && repos[0]?.defaultBranch === "trunk",
  );
  ...
  // A nested group has slashes in its path; unencoded they would be read
  // as extra path segments and the call would 404.
  check("a nested project path is URL-encoded", branchUrl.includes("acme%2Fsub%2Fapi"));
