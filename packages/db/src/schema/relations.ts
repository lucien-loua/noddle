...
export const databasesRelations = relations(databases, ({ many, one }) => ({
  ...
  deployments: many(databaseDeployments),
...
export const databaseDeploymentsRelations = relations(
  databaseDeployments,
  ({ one, many }) => ({
    database: one(databases, {
      fields: [databaseDeployments.databaseId],
      references: [databases.id],
    }),
    logs: many(databaseDeploymentLogs),
  })
);

export const databaseDeploymentLogsRelations = relations(
  databaseDeploymentLogs,
  ({ one }) => ({
    deployment: one(databaseDeployments, {
      fields: [databaseDeploymentLogs.databaseDeploymentId],
      references: [databaseDeployments.id],
    }),
  })
);
