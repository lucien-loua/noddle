...
import { BACKUP_EXTENSION, buildBackupInsert, joinBackupPrefix, pickDestination } from "#index";
...
await runVerify("backup domain", () => {
  ...
  expectThrows(
    ...
    (e) => e instanceof Error && e.message.includes("no S3 destination configured"),
  );

  const single = pickDestination([a], null);
  check("single destination: returned without needing to choose", single.id === "a");
  ...
  check("two destinations, explicit choice: returns the REQUESTED one", chosen.id === "b");
  ...
  check(
    ...
    insertC.configId === "cfg-1" && insertC.objectKey.startsWith("dest-a/nightly/shop_db/"),
    JSON.stringify(insertC),
  );

  const hourly = "0 * * * *";
  const now = new Date("2026-08-12T12:30:00.000Z");
  check("isConfigDue: first run when nothing completed yet", isConfigDue(hourly, null, now));
  check(
  ...
  check("isConfigDue: invalid cron returns false", !isConfigDue("not a cron", null, now));
});
