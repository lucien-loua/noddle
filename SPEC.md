...
Package manager: pnpm, monorepo via pnpm workspaces (no need for Turborepo/Nx at this scale unless you split into many packages).
...
deployment_logs
  id, deployment_id, chunk, created_at   -- or stream straight from job to SSE + persist to object storage/file, don't row-per-line in Postgres past a size threshold
...
`install.sh` responsibilities, run via `curl -fsSL https://get.yourapp.dev | bash`:

1. Detect OS (Ubuntu/Debian primarily, warn on others).
2. Install Docker + Docker Compose plugin if missing.
3. Create `/etc/yourapp` for config, generate `APP_KEY` (encryption key) if not present.
...
So: **run every build in a resource-capped container** (`--memory`, `--cpus`, hard limits derived from server capacity), so a build can never starve running services. Surface it as one line in the UI — "builds capped at X GB" — not a settings panel.
