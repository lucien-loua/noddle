...
- **`Checkbox` carries `border-ring` at rest, and `shadcn add checkbox` will delete it** (2026-09-07). The preset ships `border-transparent bg-input/90`, so an UNCHECKED box is only `--input`: measured 1.23:1 against a white surface and 1.48:1 against the dark one, where WCAG 1.4.11 asks 3:1 for a control's boundary. It is invisible, not merely subtle. `--border` does not rescue it — it measures 1.26:1 in BOTH themes. `--ring` is the lightest token that clears the bar on both grounds (3.64 light, 3.78 dark), and it is the interactive-boundary token, so that is the one. Focus stays distinguishable because `focus-visible` adds a 3px halo on top. Same hazard as `Progress` below: the file is regenerated, so re-apply after any `shadcn add`.
