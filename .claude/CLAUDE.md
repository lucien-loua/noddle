...
Read `CONTEXT.md` and relevant ADRs before exploring or proposing alternatives. If an ADR looks wrong, say so once and stop — do not work around it.
...
`needs-triage` · `needs-info` · `ready-for-agent` · `ready-for-human` · `wontfix` as `Status:` lines. See `docs/agents/triage-labels.md`.

### Domain docs

Single-context: `CONTEXT.md` + `docs/adr/`. This file is operating instructions only. See `docs/agents/domain.md`.
...
Practical consequence of ADR-0014: **every new file is AGPL.** No incompatible licenses.
...
Canonical: **ADR-0016**. Targets are **Multipass VMs** over real SSH — not DinD, not OrbStack.
