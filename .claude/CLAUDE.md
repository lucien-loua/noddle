...
- **Controls carry NO resting border, and that is the system — do not "fix" the checkbox** (2026-09-07, tried and reverted at the user's call). `Checkbox`, `Input`, `Textarea` and `SelectTrigger` all ship `bg-input/… border-transparent`, with `border-ring` reserved for focus and `border-destructive` for invalid. Measured, an unchecked box is 1.23:1 against a white surface and 1.48:1 against the dark one, where WCAG 1.4.11 asks 3:1 for a control boundary, so the finding is real and will be found again. It is a property of `--input`, shared by every control, NOT of the checkbox: `--border` is the same `oklch(0.922)` and measures 1.26:1 in both themes. Patching one component was rejected for two reasons — it gives one control a resting border its siblings lack, and it spends `border-ring` on the resting state, so the colour that meant "focused" starts meaning "exists". If this is ever taken up, it is one deliberate change to the palette across every control, not a per-component border.
